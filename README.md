...
- **Unreal Engine 5.3+** with `PythonScriptPlugin` and `GameplayAbilities` (both ship with the engine). The matrix at `tools/build_matrix.py` verifies clean BuildPlugin against 5.3.2 / 5.4.4 / 5.5.4 / 5.6.1 / 5.7.1 / 5.8.0; some libraries (Chooser / PoseSearch / Material / Navigation / StateTree / Smart Object + a few standalone UFUNCTIONs) require 5.7+, and a handful of inline shims cover 5.3 and 5.8 — see [docs/version-compatibility.md](docs/version-compatibility.md). UE 5.2 and earlier are not supported.
